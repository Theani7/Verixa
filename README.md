...
nobody else can overwrite or delete them, even with the id. Usernames are
suggested from your email at signup and can be changed in Settings. Share links
stay public so recipients need no account. Signing in also claims your
existing local threads for your account.

## Settings

The profile menu (avatar, bottom of the sidebar) opens Settings with three
sections. Account edits your full name and username, shows your login email
(read-only) and join date, changes the password, signs out, and links to
...
option reaches the answer prompt and visibly changes it.
